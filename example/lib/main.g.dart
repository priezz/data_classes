...
class Fruit extends IDataClass<Fruit, FruitModel> {
  ...
  factory Fruit.from(Fruit source) =>
      Fruit.build((b) => b.withValuesFrom(source._model));

  factory Fruit.fromModel(FruitModel source) =>
      Fruit.build((b) => b.withValuesFrom(source));

  Fruit.build(DataClassBuilder<_FruitModelBuilder>? build) {
    final builder = _FruitModelBuilder(_model);
    build?.call(builder);
    _updateModel(builder.build());
  }
  ...
  @override
  Fruit copy([DataClassBuilder<_FruitModelBuilder>? update]) =>
      Fruit.build((builder) {
        update?.call(builder.withValuesFrom(_model));
        _notifyOnPropChanges(_model, builder.build());
      });

  @override
  Future<Fruit> copyAsync(
      [DataClassAsyncBuilder<_FruitModelBuilder>? update]) async {
    final builder = _FruitModelBuilder(_model);
    await update?.call(builder);
    return Fruit.fromModel(builder.build());
  }
  ...
  @override
  FruitModel get thisModel => _model;

  void _updateModel(FruitModel updated) => _model
    ..color = updated.color
...
class Tree extends IDataClass<Tree, TreeModel> {
  ...
  factory Tree.from(Tree source) =>
      Tree.build((b) => b.withValuesFrom(source._model));

  factory Tree.fromModel(TreeModel source) =>
      Tree.build((b) => b.withValuesFrom(source));

  Tree.build(DataClassBuilder<_TreeModelBuilder>? build) {
    final builder = _TreeModelBuilder(_model);
    build?.call(builder);
    _updateModel(builder.build());
  }
  ...
  @override
  Tree copy([DataClassBuilder<_TreeModelBuilder>? update]) =>
      Tree.build((builder) {
        update?.call(builder.withValuesFrom(_model));
      });

  @override
  Future<Tree> copyAsync(
      [DataClassAsyncBuilder<_TreeModelBuilder>? update]) async {
    final builder = _TreeModelBuilder(_model);
    await update?.call(builder);
    return Tree.fromModel(builder.build());
  }
  ...
  @override
  TreeModel get thisModel => _model;

  void _updateModel(TreeModel updated) => _model
    ..name = updated.name
    ..averageHeight = updated.averageHeight;
}
