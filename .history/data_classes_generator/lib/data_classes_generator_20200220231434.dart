...
const builderClassSuffix = 'Builder';
...
class DataClassGenerator extends GeneratorForAnnotation<GenerateDataClass> {
  ...
  generateForAnnotatedElement(
    ...
    final name = originalClass.name
        .substring(0, originalClass.name.length - builderClassSuffix.length);
    ...
    for (final field in fields) {
      ...
      if (enumClass?.isEnum == false) {
        throw CodeGenError(
            'You annotated the $name$builderClassSuffix\'s ${field.name} with '
        ...
        for (final negate in generateNegations ? [false, true] : [false]) {
          ...
          if (valueGetters.containsKey(getter)) {
            throw CodeGenError(
                ...
                'conflicting value getters of the $name$builderClassSuffix class are:\n'
    ...
    buffer.writeAll([
      ...
      '/// {@nodoc}',
      'typedef ${name}Fn = Function($name$builderClassSuffix b);',

      /// Start of the class.
      '/// {@nodoc}',
      if (immutable)
        '@immutable',
      'class $name$builderClassSuffix extends $name {',
      // '$name$builderClassSuffix _model;\n',
      ...
      '}) {',
      'final model = $name$builderClassSuffix();',
      'return $name._(',
      for (final field in fields)
      ...
      '/// Creates a [$name] from a [$name$builderClassSuffix].',
      '$name.fromMutable($name$builderClassSuffix mutable, {void Function($name$builderClassSuffix source) update,}) {',
      '_model = $name$builderClassSuffix()',
      fields
      ...
      'return identical(this, other) || other is $name$builderClassSuffix &&',
      ...
      '$name copy(void Function($name$builderClassSuffix source) update) {',
      'assert(update != null,',
      ...
      if (serialize) ...[
        ...
        '$name.fromMutable(_\$$name${builderClassSuffix}FromJson(json));\n',

        // toJson
        'Map<dynamic, dynamic> toJson() =>',
        '_\$$name${builderClassSuffix}ToJson(this.toMutable());\n',
      ...
      if (serialize && builtValueSerializer) ...[
        ...
        '    final json = _\$$name${builderClassSuffix}ToJson(object);',
        ...
        '    });\n',
        '    return $name.fromMutable(_\$$name${builderClassSuffix}FromJson(json),);',
        '  }\n',
        '}',
      ]
    ].expand((line) => [line, '\n']));
