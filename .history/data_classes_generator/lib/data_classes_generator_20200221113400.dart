...
class DataClassGenerator extends GeneratorForAnnotation<GenerateDataClass> {
  ...
  generateForAnnotatedElement(
    ...
    if (!element.name.startsWith('_') ||
        !element.name.endsWith(builderSuffix)) {
      throw CodeGenError(
    ...
    final name = originalClass.name
        .substring(1, originalClass.name.length - builderSuffix.length);
    ...
    buffer.writeAll([
      ...
      '/// {@nodoc}',
      'typedef ${name}Updater = void Function($name$builderSuffix);',
      ...
      'factory $name$builderSuffix([${name}Updater update]) => $name$builderSuffix._($name._(), update);',
      '$name$builderSuffix._($name source, [${name}Updater update]):',
      ...
      '\'the original.\',',
      ');',
      '\nreturn $name$builderSuffix._(this, update);',
      '}\n',
      ...
      if (serialize) ...[
        ...
        '$name$builderSuffix._(_\$${name}FromJson(json));\n',
      ...
      if (serialize && builtValueSerializer) ...[
        ...
        '    });\n',
        '    return $name$builderSuffix._($name._(), _\$${name}FromJson(json),);',
        '  }\n',
        '}',
      ]
    ].expand((line) => [line, '\n']));
